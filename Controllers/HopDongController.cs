...
namespace QuanLiPhongTro.Controllers
{
    ...
    public class HopDongController : Controller
    {
        ...
        public async Task<IActionResult> DanhSach(HopDongFilterViewModel filter)
        {
            ...
            if (!string.IsNullOrEmpty(filter.ToaNha))
                query = query.Where(h => h.Phong.ToaNha.TenToa == filter.ToaNha);

            if (!string.IsNullOrEmpty(filter.Phong))
                query = query.Where(h => h.Phong.TenPhong == filter.Phong);

            if (filter.TrangThai.HasValue)
                query = query.Where(h => h.DaTra == filter.TrangThai.Value);

            if (filter.Thang.HasValue)
            {
                var thang = filter.Thang.Value;
                query = query.Where(h => h.NgayBatDau.Month == thang.Month && h.NgayBatDau.Year == thang.Year);
            }

            if (!string.IsNullOrEmpty(filter.TuKhoa))
            {
                query = query.Where(h =>
                    h.User.UserName.Contains(filter.TuKhoa) ||
                    h.User.PhoneNumber.Contains(filter.TuKhoa));
            }

            filter.TotalRecords = await query.CountAsync();

            filter.HopDongs = await query
                .OrderByDescending(h => h.NgayBatDau)
                .Skip((filter.Page - 1) * filter.PageSize)
                .Take(filter.PageSize)
                .ToListAsync();

            // Gửi danh sách tòa nhà và phòng (tuỳ bạn dùng ViewBag hoặc SelectList)
            ViewBag.ToaNhas = await _context.ToaNhas.Select(t => t.TenToa).Distinct().ToListAsync();
            ViewBag.Phongs = await _context.Phongs.Select(p => p.TenPhong).Distinct().ToListAsync();
        ...
        public async Task<IActionResult> Them(HopDongViewModel vm)
        {
            ...
            var phong = await _context.Phongs.FirstOrDefaultAsync(p => p.Id == vm.PhongId);
            if (phong == null || phong.DaChoThue)
            {
                ModelState.AddModelError("PhongId", "Phòng không hợp lệ hoặc đã cho thuê");
                await LoadDropdownData(vm);
                return View(vm);
            }
            ...
            try
            {
                ...
                if (await _context.NguoiThues.FindAsync(vm.UserId) == null)
                {
                ...
                var thanhToan = new ThanhToan
                {
                    ...
                    trangThaiThanhToan = TrangThaiThanhToan.ChuaThanhToan,
                    NgayThanhToan = DateTime.Now,
                    TongTien = phong.GiaTien * 6 + vm.TienCoc,
                    ThangNam = DateTime.Now
                };
                ...
                _context.ChiTietThanhToans.AddRange(
                    new ChiTietThanhToan
                    {
                        ThanhToanId = thanhToan.Id,
                        Loai = "TienPhong",
                        MoTa = $"Thanh toán tiền phòng cho phòng {phong.TenPhong}",
                        SoTien = phong.GiaTien * 6
                    },
                    new ChiTietThanhToan
                    {
                        ThanhToanId = thanhToan.Id,
                ...
                await DangKyDichVuDienNuoc(vm.PhongId);
                await transaction.CommitAsync();
            ...
            {
                await transaction.RollbackAsync();
                ModelState.AddModelError("", "Lỗi khi lưu hợp đồng: " + ex.Message);
                await LoadDropdownData(vm);
                return View(vm);
            }
        }

        private async Task DangKyDichVuDienNuoc(string phongId)
        {
            var dvDien = await _context.DichVus.FirstOrDefaultAsync(d => d.TenDichVu == "Điện");
            var dvNuoc = await _context.DichVus.FirstOrDefaultAsync(d => d.TenDichVu == "Nước");
